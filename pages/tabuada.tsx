...
export default function Tabuada() {
  ...
  const [tabuada, setTabuada] = useState<String[]>([]);

  const Calcular = () => {
    try {
      if (!isNaN(firstNumber)) {
        const tabuadaString: String[] = [];
        for (let i = 1; i < 11; i++) {
          tabuadaString.push(`${firstNumber} x ${i} = ${firstNumber * i}`);
        }
        setTabuada(tabuadaString);
      }
    } catch (error) {
      console.log(error);
    }
