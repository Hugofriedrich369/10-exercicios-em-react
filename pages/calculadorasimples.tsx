...
export default function CalculadoraSimples() {
  ...
  async function setarPrimeiroNumero(valor: number) {
    try {
      let valorFunc = valor;
      await setFirstNumber(valor);
      if (Number.isNaN(valorFunc)) valorFunc = 0;
      setResult(`${valorFunc} ${simbolo} ${secondNumber}`);
    } catch (error) {
      console.log(error);
    }
  }
  async function setarSegundoNumero(valor: number) {
    try {
      let valorFunc = valor;
      await setSecondNumber(valor);
      if (Number.isNaN(valorFunc)) valorFunc = 0;
      setResult(`${firstNumber} ${simbolo} ${valorFunc}`);
    } catch (error) {
      console.log(error);
    }
  }

  async function MostrarCalculo(simboloString: String) {
    try {
      setSimbolo(`${simboloString}`);
      setResult(`${firstNumber} ${simboloString} ${secondNumber}`);
    } catch (error) {
      console.log(error);
    }
  }
  ...
  const Calcular = () => {
    try {
      if (firstNumber && secondNumber) {
        if (simbolo == "+") setResult((firstNumber + secondNumber).toString());
        else if (simbolo == "-")
	...
	C512,114.609,397.391,0,256,0z M256,472c-119.297,0-216-96.703-216-216S136.703,40,256,40s216,96.703,216,216S375.297,472,256,472z"
		...
		c4.358-4.203,7.922-9.25,10.547-15.234c2.734-5.906,4.047-12.5,4.047-19.625H284v256h-34.297V201.25z"
            ...
            <input
              ...
              onChange={(e) => setarPrimeiroNumero(e.target.valueAsNumber)}
            />
          </div>
          <div className="mt-5">
            <label
	...
	c-119.297,0-216-96.703-216-216S136.703,40,256,40s216,96.703,216,216S375.297,472,256,472z"
		...
		c-15.359,0-27.531,4.703-36.453,14.109c-9,9.375-13.438,22.25-13.438,38.625H176z"
              ...
              <input
                ...
                id="second_number"
                className="rounded-none rounded-r-lg bg-gray-50 border text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                onChange={(e) => setarSegundoNumero(e.target.valueAsNumber)}
              />
            </div>
            <div className="mt-5 grid grid-cols-4 gap-5">
              <button
                type="button"
                onClick={(e) => MostrarCalculo("+")}
            ...
            <div className="mt-5 grid grid-cols-2 gap-5">
              <div className="flex">
                <button
                  type="button"
                  className="grow text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2"
                  onClick={Calcular}
                >
                  Calcular
                </button>
              ...
              <div>
                <Link
                  href={"/"}
                  type="button"
                  className="grow text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center"
                >
                  Voltar ao início
                  <svg
